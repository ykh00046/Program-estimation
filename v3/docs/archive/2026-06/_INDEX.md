...
## PDCA #37 (Recipe SSOT Unification — 배합 레시피 DB 일원화 + 편집 화면)

| 문서 | 원래 위치 | 단계 |
| --- | --- | --- |
| [recipe_ssot_unification.plan.md](./recipe_ssot_unification/recipe_ssot_unification.plan.md) | docs/01-plan/features/ (루트) | Plan |
| [recipe_ssot_unification.design.md](./recipe_ssot_unification/recipe_ssot_unification.design.md) | docs/02-design/features/ (루트) | Design |
| [recipe_ssot_unification.analysis.md](./recipe_ssot_unification/recipe_ssot_unification.analysis.md) | docs/03-analysis/ (루트) | Analysis (Match **100%** 1차 통과) |
| [recipe_ssot_unification.report.md](./recipe_ssot_unification/recipe_ssot_unification.report.md) | docs/04-report/ (루트) | Report |
...
- **배합 레시피 SSOT 일원화 (#37)**: 레시피 진실을 Excel 파일 → DB(recipes 테이블)로 전환(빈 테이블 1회 자동 시드), 죽어 있던 RecipeRepository 부활, 신규 RecipeEditDialog로 **처음으로 프로그램 안에서 배합 레시피 편집 가능**. 364 passed (+18), 커밋 8539d8c.
- **기록 조회 임베드 + 상태바 겹침 수정 (#38, 경량)**: 기록 조회를 별도 창 → 사이드바 페이지 직접 표시(재진입 새로고침), 상태바 "준비됨" 겹침 해소(main_label 단일 경로). 371 passed (+7), 커밋 eaef1ff. 웹 전환 질문은 데스크톱 유지로 합의. (수동 QA 중 빌드 PATH 오염 사고 수정 7c5d1b8 포함 — bare pyinstaller → sys.executable -m 고정)
- **아카이브 날짜**: 2026-06-01 (#27), 2026-06-10 (#33, #34, #35, #36, #37)
